...
```bash
git -C "$MEMENTO_ROOT" add -- <session-file> [<private-file>]
git -C "$MEMENTO_ROOT" commit -m "ama: capture interview — <slug>"
```

Stage **only the files this interview wrote** (the session capture and any private
append) — never `git add sources/` broadly, which
